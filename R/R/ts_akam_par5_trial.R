#' @templateVar MODEL_FUNCTION ts_akam_par5_trial
...
#' @templateVar MODEL_CODE akam_par5_trial
#' @templateVar MODEL_CITE (Akam et al., 2015)
#' @templateVar MODEL_TYPE Hierarchical
#' @templateVar DATA_COLUMNS "subjID", "level1_choice", "level2_choice", "reward"
#' @templateVar PARAMETERS \code{a1} (learning rate in stage 1), \code{beta1} (inverse temperature in stage 1), \code{a2} (learning rate in stage 2), \code{pi} (perseverance (unsigned, if signed, so positive and negative/stay and switch, put to -5, 0, 5)), \code{w} (model-based weight)
...
ts_akam_par5_trial <- hBayesDM_model(
  task_name       = "ts",
